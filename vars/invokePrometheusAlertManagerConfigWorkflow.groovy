...
def call(String application, String configuration) {
    ...
    timestamps {
        ...
        if (env.BRANCH_NAME =~ /^master/) {
            ...
            prometheusAlertManagerConfigWorkflow(PrometheusContext, 'master', scmVars,true)
            ...
            prometheusAlertManagerConfigWorkflow(PrometheusContext, env.BRANCH_NAME, scmVars, true)
        } else if (env.BRANCH_NAME =~ /(BH|PC|HD|SLR)-\d*/) {
            echo("[Prometheus alert Manager Pipeline] feature  branch being built:" + application)
            prometheusAlertManagerConfigWorkflow(PrometheusContext, env.BRANCH_NAME, scmVars, false)
        } else {
