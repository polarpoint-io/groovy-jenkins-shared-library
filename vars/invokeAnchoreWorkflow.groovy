...
def call(String application, String configuration) {
    ...
    node('master') {
        ...
        echo("[Generic Workflow] calling  with configuration:" + ( new JsonBuilder( anchoreContext ).toPrettyString()))

            if (env.BRANCH_NAME =~ /^master/) {
                echo("[Generic Workflow] master branch being build and tagged:" + application)
                anchoreWorkflow(anchoreContext, 'master')
